...
public class PlacementTest {
    ...
    private class DebugPlacement extends DefaultDataMatrixPlacement {
        ...
        public String toBitFieldString() {
            StringBuffer sb = new StringBuffer(this.bits.length);
            for (int i = 0, c = this.bits.length; i < c; i++) {
                sb.append(this.bits[i] == 1 ? '1' : '0');
            }
            return sb.toString();
        }
        ...
        public String[] toBitFieldStringArray() {
            ...
            for (int row = 0; row < this.numrows; row++) {
                StringBuffer sb = new StringBuffer(this.bits.length);
                for (int i = 0, c = this.numcols; i < c; i++) {
                    sb.append(this.bits[startpos + i] == 1 ? '1' : '0');
                }
                array[row] = sb.toString();
                startpos += this.numcols;
            }
            return array;
        }
