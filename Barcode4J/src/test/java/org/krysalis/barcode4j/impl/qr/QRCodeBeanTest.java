...
public class QRCodeBeanTest {
    ...
    public void testParameters() {
        ...
        Assert.assertEquals(20, bean.getMaxSize().width);
        bean.setMaxSize(null);
        Assert.assertNull(bean.getMaxSize());

        bean.setMinSize(new Dimension(24, 24));
        Assert.assertEquals(24, bean.getMinSize().width);
        bean.setMinSize(null);
        Assert.assertNull(bean.getMinSize());

        Assert.assertEquals("ISO-8859-1", bean.getEncoding()); //the default
        try {
            bean.setEncoding("DoesNotExist");
            Assert.fail("Need exception on bad encoding");
        } catch (IllegalArgumentException iae) {
            //expected
        }
        bean.setEncoding("UTF-8");
        Assert.assertEquals("UTF-8", bean.getEncoding());

        Assert.assertEquals('L', bean.getErrorCorrectionLevel());
        try {
            bean.setEncoding("Z");
            Assert.fail("Need exception on bad error correction level");
        } catch (IllegalArgumentException iae) {
            //expected
        }
        bean.setErrorCorrectionLevel('M');
        Assert.assertEquals('M', bean.getErrorCorrectionLevel());
    }
