...
public class ConfigurationUtilTestCase {
    ...
    private static void checkCfgTree(final Configuration cfg) throws Exception {
    	Assert.assertNotNull(cfg);
    	Assert.assertEquals("root", cfg.getName());
    	Assert.assertEquals("value", cfg.getAttribute("name"));
    	Assert.assertNull(cfg.getValue(null));
        Configuration childcfg = cfg.getChild("child");
        Assert.assertNotNull(childcfg);
        Assert.assertEquals("child", childcfg.getName());
        Assert.assertEquals("bar", childcfg.getAttribute("foo"));
        Assert.assertEquals("hello", childcfg.getValue());
    }
