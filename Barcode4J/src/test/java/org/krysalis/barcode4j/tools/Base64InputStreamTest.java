...
public class Base64InputStreamTest {
    ...
    public void testDecoder() throws Exception {
        for (int mode = MODE_BUF; mode <= MODE_BYTE; mode++) {
            Assert.assertEquals("sure.", decode("c3VyZS4=", mode));
            Assert.assertEquals("asure.", decode("YXN1cmUu", mode));
            Assert.assertEquals("easure.", decode("ZWFzdXJlLg==", mode));
            Assert.assertEquals("leasure.", decode("bGVhc3VyZS4=", mode));
        }

        //Checks spaces and CR/LF combinations
        Assert.assertEquals("leasure.", decode("bGVh c3Vy Z\r\nS\n4="));

        //Alternative URL-safe encoding
        Assert.assertEquals("~Test~", decode("flRlc3R+"));
        Assert.assertEquals("~Test~", decode("flRlc3R-"));
    ...
    public void testInvalid() throws Exception {
        ...
        Assert.assertEquals("sur", decode("c3VyZS4"));
    ...
    private static String decode(String encoded, int mode) throws IOException {
        if (DEBUG) {
