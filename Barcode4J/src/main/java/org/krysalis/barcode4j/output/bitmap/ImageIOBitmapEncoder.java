...
public class ImageIOBitmapEncoder implements BitmapEncoder {
    ...
    private static IIOMetadata setupMetadata(BufferedImage image, ImageWriter writer,
                String mime, int resolution) throws IOException {
        IIOMetadata iiometa;
        try {
    ...
    private static void checkWritable(IIOMetadata iiometa) throws IOException {
        if (iiometa.isReadOnly()) {
    ...
    private static void dumpMetadata(IIOMetadata iiometa) {
        String[] metanames = iiometa.getMetadataFormatNames();
