...
public class DefaultCode128Encoder implements Code128Encoder, Code128Constants {
    ...
    private static boolean needA(char c) {
        //Character can't be encoded in B
        return (c < 32);
    }

    private static boolean needB(char c) {
        //Character can't be encoded in A
        return (c >= 96) && (c < 128);
    }

    private static int encodeAorB(char c, int codeset) {
        //Function chars
     ...
     */
    private static int encodeC(String message, int start, int finish, int[] encoded,
            int startEncodedPos) {
