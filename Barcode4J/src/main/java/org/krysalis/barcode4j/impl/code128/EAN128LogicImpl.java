...
public class EAN128LogicImpl { //extends Code128LogicImpl{
    ...
    protected void setMessage(String msg) {
        if (msg == null || !msg.equals(this.msgCache)) {
            ...
            if (msg == null) {
                ...
                this.code128Msg.append(Code128LogicImpl.FNC_1);
                addAIs(msg);

                Code128Encoder encoder = new DefaultCode128Encoder();
                this.encodedMsg = encoder.encode(getCode128Msg());
            }
    ...
    private int findGroupSeparator(String msg, int start) {
        ...
        if (this.groupSeparator == Code128LogicImpl.FNC_1) {
            return retGS;
        }
...
//        byte type = ai.type[0];
        ...
        this.code128Msg.append(msg.substring(offset, offset + lenID)); //BUG fixed 15.08!!
        if (!this.omitBrackets) {
            this.humanReadableMsg.append(')');
        }
...
//        start[1] = offset + lenID;
        ...
        if (newOffset < offset + lenID + lenMin) {
            if (this.checksumADD && ai.canDoChecksumADD
...
//            code128Msg.append(Code128LogicImpl.FNC_1);
    ...
    private boolean isGroupSeparator(char ch) {
        return (ch == this.groupSeparator || ch == Code128LogicImpl.FNC_1);
    }
    ...
    private IllegalArgumentException getException(String text, String msgOk) {
        ...
        if (this.humanReadableMsg.length() > 1 || msgOk.length() > 0) {
            text = text + " Accepted start of Message: \""
                + this.humanReadableMsg.toString() + msgOk + "\"";
        }
    ...
    /**
     * @return char check digit marker
     */
    ...
    /**
     * @return boolean whether brackets are omitted
     */
