...
public class FourStateLogicHandler 
	...
	public void addBar(boolean black, int height) {
        final double w = this.bcBean.getBarWidth(1);
        final double h = this.bcBean.getBarHeight(height);
        
        final double middle = this.bcBean.getBarHeight() / 2;
        double y1;
        switch (height) {
        case 0:
        case 2:
            y1 = middle - (this.bcBean.getBarHeight(0) / 2);
            break;
        case 1:
        case 3:
            y1 = middle - (this.bcBean.getBarHeight(3) / 2);
            break;
        default:
            throw new RuntimeException("Bug!");
        }
        
        this.canvas.drawRectWH(this.x, getStartY() + y1, w, h);
