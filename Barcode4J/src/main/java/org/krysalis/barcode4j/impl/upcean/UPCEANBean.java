...
public abstract class UPCEANBean extends AbstractBarcodeBean {
    ...
    protected double supplementalWidth(String msg) {
        ...
        if (suppLen > 0) {
            ...
            width += 4 * this.moduleWidth; //left guard
            width += suppLen * 7 * this.moduleWidth;
            width += (suppLen - 1) * 2 * this.moduleWidth;
        }
        return width;
    }
	...
	public BarcodeDimension calcDimensions(String msg) {
        double width = 3 * this.moduleWidth; //left guard
        width += 6 * 7 * this.moduleWidth;
        width += 5 * this.moduleWidth; //center guard
        width += 6 * 7 * this.moduleWidth;
        width += 3 * this.moduleWidth; //right guard
        width += supplementalWidth(msg);
        final double qz = (hasQuietZone() ? this.quietZone : 0);
